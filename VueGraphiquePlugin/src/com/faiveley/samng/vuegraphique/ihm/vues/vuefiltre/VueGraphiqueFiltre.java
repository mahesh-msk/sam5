...
public class VueGraphiqueFiltre extends ViewPart implements PropertyChangeListener, ISaveablePart2 {
	...
	protected void onFilterApplied(PropertyChangeEvent evt) {
		...
		IWorkbenchPartReference myView = page.findViewReference("SAMNG.Vue.Filtre.GraphiqueVueFiltre");
		page.setPartState(myView, IWorkbenchPage.STATE_MINIMIZED);

		try {
			//Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().hideView(Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().findView("SAMNG.Vue.Filtre.GraphiqueVueFiltre"));
			Activator.getDefault().getWorkbench().getActiveWorkbenchWindow().getActivePage().showView("SAMNG.Vue.Graphique");
