 ...
 * </dl>
 */
public class VbvsCombo extends Composite {
	...
	java.util.List<ComboPopupShowListener> popupListeners = new ArrayList<ComboPopupShowListener>(0);
	Font font;
 ...
 * @see Widget#getStyle()
 */
public VbvsCombo (Composite parent, int style) {
	...
	listener = new Listener () {
		public void handleEvent (Event event) {
			...
			if (VbvsCombo.this == event.widget) {
				comboEvent (event);
				return;
			}
	...
	filter = new Listener() {
		public void handleEvent(Event event) {
			...
			if (shell == VbvsCombo.this.getShell ()) {
				handleFocus (SWT.FocusOut);
			}
		}
	};
...
public void add (String string, int index) {
	add(string, getDisplay().getSystemColor(SWT.COLOR_BLACK), index);
}
...
public void addKeyTextListener(KeyListener listener) {
	        ...
	        case SWT.MouseHover: {
	          ...
	          if (item != null) {
	            if (tip != null && !tip.isDisposed())
	              tip.dispose();
	            tip = new Shell(popup, SWT.ON_TOP | SWT.TOOL);
	            ...
	            Point pt = table.toDisplay(event.x, rect.y + size.y + 12);
...
void dropDown (boolean drop) {
	...
	for(ComboPopupShowListener listener: popupListeners)
...
//			textTblItem.setText(dropTblItem.getText());
			text.setForeground(dropTblItem.getForeground());
			text.setText (dropTblItem.getText());
			if (getEditable ()) text.selectAll ();
