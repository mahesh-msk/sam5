...
public class VueInfosParcours extends ViewPart implements ICapturable,IRepereChangedListener,IDataChangedListener {
	...
	/**
	 * D�finit le texte concernant les informations parcours
	 * 
	 */
	private void definirTexteInfosParcours() {
		...
		//nouveau calcul duree cumulee et distance cumulee utilisant le meme calcul que la r�f�rence
	...
	private static HashMap<String, Object> calculerTempsEtDistanceCumulee(int msgStartId, int msgEndId) throws Exception {
		...
		if(parseur instanceof ParseurParcoursTomUk||parseur instanceof ParseurParcoursTomDIS || parseur instanceof ParseurParcoursTomHSBC){
			...
			// parcours des messages pour r�cup�rer les d�calages relatifs aux
			...
			for (Message msg :listeMessages) {
				...
				if (relVar != null) {
					...
					if (msg.getEvenement().isChangementHeure()) {
						...
						// calcul du d�calage lors d'un changement d'heure
				...
				if (relVar != null) {
					try {
						if (!msg2.getEvenement().isChangementHeure()) {
							if (lastMsg == null) {
								...
								if(msg2.getEvenement().isRazCompteurTemps()){
									...
									if (msg2.getVariable(TypeRepere.date.getCode()) != null) {
										...
										if(msg2.getEvenement().isRuptureAcquisition()){
											date = ConversionTemps
													.getDateFromTypeRepereDate(msg2,
															TypeRepere.date);
											//										 nombre de milliseconde depuis le d�but de
											// la journ�e de la variable date
										...
										// r�cup�ration de la valeur du compteur de
										// temps
										if (msg2.getVariable(TypeRepere.temps
												.getCode()) != null) {
								...
								// si l'on change de date, on doit incr�menter
								// le compteur/d�cr�menter le compteur
								// d'autant de jour
								// il faut que la date soit valoris�e
								...
								// on met en m�moire les valeurs des variables
								// temps
								tamponDate = date;
								tamponTemps = temps;
							}
							...
							//pour les fichiers hsbc, la variable temps n'est pas valoris�e mais  la variable tempsAvantChangement est valoris�e et on doit l'utiliser en guise de compteur temps
							if (msg2.getVariable(TypeRepere.tempsAvantChangement
								...
								* (long) (ConstantesParcoursTom4.resolutionTemps * 1000);
								...
								//pour les fichiers uk, il n'y a pas de tempsAvantChangement, mais la variable temps est elle valoris�
								if (msg2.getVariable(TypeRepere.temps
										.getCode()) != null) {
									compteurTemps = (Long) msg2.getVariable(
											TypeRepere.temps.getCode())
				...
				if (relVar != null) {
					...
					if (!msg.getEvenement().isChangementHeure()) {
						...
						if (lastMsg == null) {
							...
							// r�cup�ration des date et temps du message courant
							long temps = (Long) (msg.getVariable(codeTemps).getCastedValeur());
							...
							// r�cup�ration des date et temps du message pr�c�dent
							...
							// si l'on change de date, on doit incr�menter le
							// compteur/d�cr�menter le compteur
							// d'autant de jour
							// il faut que la date soit valoris�e
							if (date != -1 && dateLastMsg != -1
									&& (date == dateLastMsg + 1))
	...
	/**
	 * M�thode de calcul du cumul des temps et distance
	 * @return
	 * @throws Exception 
	 */
