...
public class ParseurParcoursTomDIS extends AParseurParcoursTom4{
	 ...
	 * 
	 */
	protected ParseurParcoursTomDIS() {
	...
	public static AParseurParcoursTom4 getInstance() {
		try{
			if(instance==null)
				instance = new ParseurParcoursTomDIS();
			
			
			if (ActivationExplorer.getInstance().isActif()) {
				return ParseurParcoursTomDISExplorer.getInstance();
			}
			
			return (ParseurParcoursTomDIS)instance;
		}
		catch(ClassCastException ex){
			if (ActivationExplorer.getInstance().isActif()) {
				return new ParseurParcoursTomDISExplorer();
			}
			return new ParseurParcoursTomDIS();
		}
	}
	...
	protected AParcoursComposant chargerEntete() throws AExceptionSamNG {
		...
		if(nomFichier.endsWith(".ltb".toUpperCase())|| nomFichier.endsWith(".lkb".toUpperCase())|| nomFichier.endsWith(".lwb".toUpperCase())){
			ConstantesParcoursTom4.octetDebutTableEvtVar = 10138;
			ConstantesParcoursBinaire.tailleBlocData = new BigInteger(Arrays.copyOfRange(this.message, 10134, 10136)).shortValue();
		}
		...
		ConstantesParcoursTom4.resolutionTemps = 0.1;
		...
		// set the file name that is written on property file
		try {
