...
public class ParseurParcoursSamng extends ParseurParcoursBinaire {
	...
	protected AParcoursComposant chargerEntete() throws AExceptionSamNG,
		...
		// test de pr�sente du fichier xml dans le r�pertoire ressources/xml
	 ...
	 * D�but Bloc Defaut de la table Data. CL_D_3 : L�octet 5+x+8 = > le code
	...
	/**
	 * charge le CRC enregistr� de l'entete dans un objet ADonneeBlocComposant
	 */
	...
	/**
	 * charge le CRC enregistr� de la table d'assocation dans un objet
	...
	protected Message chargerMessage(int start, boolean isExploration) throws ParseurBinaireException {
		...
		int pos = 1 + 2; // 1 -> Code de d�but de block ; 2 -> ID event (sur 2 octets)
	...
	*/
	...
	private void chargerMessageEvt(Message msg, int id)
		...
		if (((DescripteurEvenement) ev.getM_ADescripteurComposant()).getNom() == null) {
			...
			tblLangues.setNomUtilisateur(Langage.FR, "Code de l'�v�nement: " //$NON-NLS-1$
					+ id);
	...
	/**
	 * R�cup�re la valeur d'une sous-variable(dans une varaible complexe)
	 * 
	 * @param v
	 *            la varaible dont on doit d�finir la valeur
	 * @param value
	...
	private int chargeVariableSurBlocksMultiple(AVariableComposant var,
		...
		int valTmp = 0;
		int valTmp2 = 0;
		// sur chaque bloc il y a syst�matiquement 2 octets attribu� au code
		// d�but et code fin
	...
	/**
	 * M�thode qui r�cup�re le nom du fichier XML avant son chargement
	 * 
	 * @return null si probl�me dans l'entete, sinon: nom du fichier XML
	 */
	...
	private static void remplirValueJRU(AVariableComposant var, int start,
		...
		try {
			...
			// position du signum � 1 :
			// sinon, si en MSB on a 0xFF suivi d'un octet avec bit 7 = 1, le 0xFF sera vu par d�faut 
			// comme bit de signe n�gatif et retir� lors du .toByteArray() car non utile pour
			// qualifier la magnitude comme �tant de signe n�gatif (bit 7 = 1 suffisant) !
			...
			// Prise en compte du cas o� le premier octet est � 0
            // car dans ce cas l� la conversion en int le supprim�
