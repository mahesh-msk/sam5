...
/**
 * 
 * @author Olivier Classe d'implémentation du parseur de donnée du parcours en
 *         fichier d'export
 */
public class ParseurExport implements IParseurInterface {
	...
	/**
	 * Méthode d'export d'une liste d'objets Row dans un fichier
	 * 
	 * @param cheminFichier
	 * @param nomsColonnes
	 * @param lignes
	...
	public void exporterLignes3(FileInputStream fis, List<String> nomsColonnes,
		...
		for (int j = 0; j < caracMaxByColumn.size(); j++) {
			...
			if (nbCaracByLigne > nbCaracteresMax) {
				// stocker les indices des colonnes où on doit changer de page
				colonnesStop.add(j);
		...
		for (int i1 = 0; i1 < nbPages; i1++) {
			// écrire les lignes avec les bons indices pour ne pas dépasser la
			// page
	...
	/**
	 * Méthode d'export d'un fichier binaire
	 * 
	 * @param cheminFichier
