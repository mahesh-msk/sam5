...
public class ParseurXML1 implements IParseurInterface {
	...
	private void transformVariableComplexe(
		...
		// on enregistre une erreur non blocante dans les d�fauts
		...
		// peut etre diff�rent de la taille de la variable complexe en bits
		// (CL_J_6 )
		// Note, la taille d'une variable complexe est donn�e en octet
		// Si la structure n'a pas une taille multiple de 8, la taille
		// ne peut �tre renseign�e dans le XML (ex : var JRU), et donc ce test  
		// ne peut se faire. De mani�re g�n�rale, si la taille de la variable
		// complexe n'est pas renseign�e, il ne faut pas faire ce test
		if (v.getDescriptor().getTailleBits() != 0)	{
			...
			// La taille en bit d'une variable complexe ne peut �tre
			// fix�e dans le XML, mais ici on peut la positionner
			v.getDescriptor().setTailleBits(totalVariablesSize);
		}
	}

	/**
	 * Methode impl�mentant la clause CL_J_7 : la combinaison des champs
	 * "byte-offset", "bit-offset" et "taille" ne peut pas aboutir � ce qune
