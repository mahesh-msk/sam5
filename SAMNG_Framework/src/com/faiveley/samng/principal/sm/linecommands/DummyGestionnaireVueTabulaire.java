...
public class DummyGestionnaireVueTabulaire extends GestionnaireVueListeBase {
	...
	protected List<ConfigurationColonne> getColonnes(VueData vueData,
		...
		if (!isCorrections) {
			for (ConfigurationColonne colonne : colCfgsList) {
				if (colonne.getNom().equals(
						TypeRepere.distanceCorrigee.getName())
						&& (!fileHasDistanceCorrections()))
					colToRemove.add(colonne);
				if (colonne.getNom().equals(TypeRepere.tempsCorrigee.getName())
						&& (!fileHasTimeCorrections())) {
					colToRemove.add(colonne);
				}
			}
		}
		...
		if(useFilter) {
			...
			if(filterName != null && !"".equals(filterName.trim())) {
				AGestionnaireFiltres filtersMng = activator.getFiltresProvider().getGestionnaireFiltres();
		...
		for (String strVar : varNames.keySet()) {
			...
			if (strVar.equals(TypeRepere.vitesseCorrigee.getName())
					|| strVar
							.equals(com.faiveley.samng.principal.sm.data.enregistrement.Messages
									.getString("NomRepere.4"))) {
				correctedSpeedPos = colCfgsList.size() - 1;
			}

		}
		List<VariableVirtuelle> listvbv = ActivatorData.getInstance()
		...
		for (VariableVirtuelle vbv : listvbv) {
			...
			if (this.mapConfigurationColonne.get(vbv.getDescriptor()
					.getM_AIdentificateurComposant().getNom()) == null) {
				// if the column does not exists, add the column to the
				// configuration
				existingColCfg = new ConfigurationColonne();

				existingColCfg.setAffiche(true);
				existingColCfg.setNom(vbv.getDescriptor()
						.getM_AIdentificateurComposant().getNom());
				existingColCfg.setLargeur(WIDTH);
				this.mapConfigurationColonne.put(vbv.getDescriptor()
						.getM_AIdentificateurComposant().getNom(),
						existingColCfg);
			}
		}
		...
		if (speedPos != -1 && correctedSpeedPos != -1) {
			// if corrected speed exists in list, remove it and insert it after
			// the speed
			colCfg = colCfgsList.remove(correctedSpeedPos);
			if (!this.listReperesRemoved.contains(GestionnaireDescripteurs
