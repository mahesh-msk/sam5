...
public class GestionLineCommandParameters {
	...
	// L'offset de la ligne de commande doit �tre actif qu'� la premi�re
	// ouverture du fichier,
	// ensuite, m�me si on ouvre le m�me fichier l'offset doit �tre inactif et
	// donc toutes les vues
	// doivent pointer sur leur premi�re ligne.
		...
		for (int i = 0; i < nbArgs; i++) {
			...
			if (args[i].equals(product) || args[i].equals(workspace)) {
				...
				if (args[i].equals(filename) || args[i].equals(filename_long)) {
					...
					// si l'argument nom de fichier n'est pas pr�sent
					if (i >= args.length - 1) {
						erreur = true;
					...
					auMoinsUnArgument = true;
					// si l'argument num�ro de message n'est pas pr�sent
					if (i >= args.length - 1) {
						erreur = true;
					...
					auMoinsUnArgument = true;
					// si l'argument n'est pas pr�sent
					if (i >= args.length - 1) {
						erreur = true;
	...
	public static void gestionFichierAbsent() {
		// si pas de fichier mentionn� et pas de demande d'infos mais un
		// param�tre ok (o, p ou r) => erreur
	...
	public static void runExport(String file) {
		try {
			...
			// Seulement si au moins un des export est s�lectionn�
			if (isExport) {
	...
	public static void runExportReport(String file) throws AExceptionSamNG, ParseurXMLException {
		loadFile(file);
		// R�cup�ration du rapport d'erreur qui est le fichier de log
