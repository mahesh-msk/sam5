...
//				System.out.println("dateRegrTpsCurrent = " + dateRegrTpsCurrent);
			...
			// M�me mission car date identique
			else{
	...
	public static void lancerParseurMissions(File f, Job job) {
		...
		try {
			System.out.println("Cr�ation du fichier XML"); //$NON-NLS-1$
...
//			System.out.println("Nom du fichier : " +f.getName()); //$NON-NLS-1$
			...
			System.out.println("Probl�me lors de la cr�ation du fichier xml des missions"); //$NON-NLS-1$
			e1.printStackTrace();
		}
		...
		// Pour les fichiers JRU, le fichier de mission est cr�� dans chargerDataExplore()
		try {
		if (!(TypeParseur.getInstance().getParser()instanceof ParseurParcoursJRU)) {
